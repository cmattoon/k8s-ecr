...
if [ -z "$AWS_ACCOUNT_ID" ]; then
    ...
    echo "Using ${AWS_ACCESS_KEY_ID} to determine account ID"
...
ECR_TOKEN=$(aws ecr get-login-password --region $AWS_DEFAULT_REGION | base64 -d)
if [ -z "$ECR_TOKEN" ]; then
...
kubectl --namespace "${ECR_SECRET_NAMESPACE}" delete secret "${ECR_SECRET_NAME}"
